...
local emb_dir = '/scratch1/huah/treelstm/data/glove/' --'data/glove/'
...
local train_dataset = similarityMeasure.read_relatedness_dataset(train_dir, vocab)
local dev_dataset = similarityMeasure.read_relatedness_dataset(dev_dir, vocab)
local test_dataset = similarityMeasure.read_relatedness_dataset(test_dir, vocab)
printf('num train = %d\n', train_dataset.size)
printf('num dev   = %d\n', dev_dataset.size)
printf('num test  = %d\n', test_dataset.size)
...
for i = 1, num_epochs do
  ...
  if dev_score >= best_dev_score then
    ...
    local model_save_path = string.format(similarityMeasure.models_dir .. '/model-%s.%dl.%dd.epoch-%d.%.5f.%d.th', args.model, args.layers, args.dim, i, test_sco, id)
