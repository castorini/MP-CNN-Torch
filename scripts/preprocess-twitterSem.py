"""
Preprocessing script for TWITTER data.
...
def javac(filepath, cp):
    cmd = '/usr/lib/jvm/java-1.8.0-oracle.x86_64/bin/javac -cp %s %s' % (cp, filepath)
    print(cmd)
    os.system(cmd)
...
def dependency_parse(filepath, cp='', tokenize=True):
    ...
    cmd = ('/usr/lib/jvm/java-1.8.0-oracle.x86_64/bin/java -cp %s DependencyParse -tokpath %s -parentpath %s -relpath %s %s < %s'
...
def constituency_parse(filepath, cp='', tokenize=True):
    ...
    tokenize_flag = '-tokenize - ' if tokenize else ''
    cmd = ('/usr/lib/jvm/java-1.8.0-oracle.x86_64/bin/java -cp %s ConstituencyParse -tokpath %s -parentpath %s %s < %s'
...
if __name__ == '__main__':
    ...
    sick_dir = os.path.join(data_dir, 'seme')
    ...
    split(os.path.join(sick_dir, 'SEME_train_lc.txt'), train_dir)
    split(os.path.join(sick_dir, 'SEME_dev_lc.txt'), dev_dir)
    split(os.path.join(sick_dir, 'SEME_test.txt'), test_dir)
